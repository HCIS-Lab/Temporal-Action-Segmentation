...
class LTCModule(nn.Module):
    def __init__(self,
        ...
        if num_slots > 0:
            ...
            self.slot_attention = SlotAttentionModule(self.num_slots, model_dim)
    ...
    def forward(self, inputs: Tensor, masks: Tensor, prev_stage_feat: Tensor = None):
        ...
        if self.num_slots > 0:
            attn, slot_feature = self.slot_attention(feature, masks)
            feature = feature + slot_feature
            # feature = torch.cat((feature, slot_feature), 1)
